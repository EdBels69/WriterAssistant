...
class StatisticsController {
  ...
  setupRoutes() {
    this.router.get('/overview/:userId', this.getOverview.bind(this))
    this.router.get('/productivity/:userId', this.getProductivity.bind(this))
    this.router.get('/sessions/:userId', this.getSessions.bind(this))
    this.router.get('/goals/:userId', this.getGoals.bind(this))
  ...
  async createGoal(req, res) {
    try {
      ...
      if (!userId || !type || !targetValue) {
        return res.status(400).json({ error: 'userId, type and targetValue are required' })
      }
